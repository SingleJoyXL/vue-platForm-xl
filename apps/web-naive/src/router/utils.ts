...
// 动态路由
import { getMenuList, type MenuItem } from "#/api/routes";
...
async function getAsyncRoutes(userId: string) {
  ...
  return dealTreeData(menuList);
}

function dealTreeData(menuList: MenuItem[], childrenLength?: number) {
  return menuList.map(item =>
    item.children && item.children.length
      ? {
          path: item.authObjectUrl,
          meta: {
            title: item.authObjectDesc,
            backstage: true,
            keepAlive: true
          },
          children: dealTreeData(item.children, item.children.length)
        }
      : childrenLength
        ? childrenLength === 1
          ? {
              path: item.authObjectUrl,
              name: item.defaultUrl,
              meta: {
                title: item.authObjectDesc,
                backstage: true,
                keepAlive: true,
    ...
    if (v.meta?.frameSrc) {
      ...
      const index = v?.component
        ...
        : modulesRoutesKeys.findIndex(ev =>
            ev.includes(
              v.path.indexOf("index") > 0 ? v.path : `${v.path}/index`
            )
          );
