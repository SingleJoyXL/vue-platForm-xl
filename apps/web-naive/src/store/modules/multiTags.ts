...
function normalizeMultiTags(tags: unknown) {
  if (!Array.isArray(tags)) return [...routerArrays];
  const validTags = tags.filter(
    tag =>
      tag &&
      typeof tag === "object" &&
      typeof (tag as multiType).path === "string"
  );
...
export const useMultiTagsStore = defineStore("pure-multiTags", {
  ...
  actions: {
    ...
    handleTags<T>(
      ...
      this.multiTags = normalizeMultiTags(this.multiTags);
      switch (mode) {
        case "equal":
          this.multiTags = normalizeMultiTags(value);
          this.tagsCache(this.multiTags);
          break;
        case "push":
          {
            const tagVal = value as multiType;
            if (!tagVal?.path) return;
            // 不添加到标签页
            if (tagVal?.meta?.hiddenTag) return;
            // 如果是外链无需添加信息到标签页
            if (isUrl(tagVal?.name)) return;
            // 如果title为空拒绝添加空信息到标签页
            if (!tagVal?.meta?.title) return;
